  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const userIssues = await db
        ...
        .from(issues)
        .where(
          and(eq(issues.authorId, user.id), gte(issues.createdAt, oneYearAgo)),
        )
      ...
      const userReviews = await db
        ...
        .where(
          and(
            eq(reviewRequests.authorId, user.id),
            gte(reviewRequests.createdAt, oneYearAgo),
          ),
        )
      ...
      for (const item of userIssues) {
        ...
        if (!dailyMap[d])
          dailyMap[d] = { count: 0, issues: 0, reviews: 0, comments: 0 };
        dailyMap[d].count += 1;
        dailyMap[d].issues += 1;
      }
