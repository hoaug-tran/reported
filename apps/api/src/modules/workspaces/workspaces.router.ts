...
workspacesRouter.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
...
workspacesRouter.patch('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
...
workspacesRouter.delete('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
...
workspacesRouter.get('/:id/members', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
    ...
    if (!membership) {
      throw new AppError(403, 'FORBIDDEN', 'Access denied to workspace members');
    }
...
workspacesRouter.post('/:id/members/invite', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
    ...
    if (existingUser) {
      ...
      if (alreadyMember) {
        throw new AppError(409, 'ALREADY_MEMBER', 'User is already a member of this workspace');
      }
...
workspacesRouter.patch('/:id/members/:userId/role', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const currentUser = req.user!;
    const { id, userId } = req.params;
    await authorization.assertCan(currentUser.id, id, 'workspace.member.role');
    ...
    if (input.role !== WorkspaceRole.OWNER) {
      ...
      if (target?.role === WorkspaceRole.OWNER) {
        const [ownerCount] = await db
          .select({ count: sql<number>`count(*)::int` })
          .from(workspaceMembers)
          .where(and(eq(workspaceMembers.workspaceId, id), eq(workspaceMembers.role, WorkspaceRole.OWNER)));

        if ((ownerCount?.count || 0) <= 1) {
          throw new AppError(400, 'CANNOT_DEMOTE_LAST_OWNER', 'Workspace must have at least one Owner');
        }
      }
    }
...
workspacesRouter.delete('/:id/members/:userId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const currentUser = req.user!;
    const { id, userId } = req.params;

    if (currentUser.id !== userId) {
      await authorization.assertCan(currentUser.id, id, 'workspace.member.remove');
    }
    ...
    if (target.role === WorkspaceRole.OWNER) {
      ...
      if ((ownerCount?.count || 0) <= 1) {
        throw new AppError(400, 'CANNOT_REMOVE_LAST_OWNER', 'Cannot remove the last Owner of the workspace');
      }
    }
...
workspacesRouter.get('/:id/projects', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
    ...
    const membership = await authorization.getWorkspaceMember(user.id, id);
    if (!membership) {
      throw new AppError(403, 'FORBIDDEN', 'Access denied to workspace projects');
    }
...
workspacesRouter.post('/:id/projects', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id } = req.params;
...
workspacesRouter.patch('/:id/projects/:projectId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
...
workspacesRouter.delete('/:id/projects/:projectId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    const { id, projectId } = req.params;
    ...
    await db.update(issues).set({ projectId: null }).where(eq(issues.projectId, projectId));
    await db.update(reviewRequests).set({ projectId: null }).where(eq(reviewRequests.projectId, projectId));
