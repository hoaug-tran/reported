...
import {
  db, notifications, notificationPreferences, users,
  eq, and, desc, sql
} from '@reported/database';
import { NotificationType, NotificationChannel, UpdateNotificationPreferencesSchema } from '@reported/contracts';
...
notificationsRouter.get('/', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const enriched = await Promise.all(items.map(async (n) => {
      const actor = n.actorId ? await db.query.users.findFirst({
...
notificationsRouter.post('/read-all', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
    await db.update(notifications)
      .set({ isRead: true, readAt: new Date() })
...
notificationsRouter.get('/preferences', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user!;
...
notificationsRouter.put('/preferences', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
