...
export class GitLabCodeHostProvider implements ICodeHostProvider {
  ...
  async listRepositories(accessToken?: string, query?: string): Promise<RepositoryItem[]> {
    ...
    try {
      ...
      if (query) {
        ...
        filtered = projects.filter((p) => p.path_with_namespace.toLowerCase().includes(q));
      }
  ...
  async listPullRequests(owner: string, repo: string, accessToken?: string): Promise<PullRequestItem[]> {
    try {
      ...
      const res = await fetch(`https://gitlab.com/api/v4/projects/${projectPath}/merge_requests?per_page=30`, { headers });
      if (!res.ok) {
        return [];
      }
  ...
  async getPullRequest(owner: string, repo: string, number: number, accessToken?: string): Promise<PullRequestItem | null> {
    try {
      ...
      const res = await fetch(`https://gitlab.com/api/v4/projects/${projectPath}/merge_requests/${number}`, { headers });
      if (!res.ok) {
        return null;
      }
  ...
  async listBranches(owner: string, repo: string, accessToken?: string): Promise<string[]> {
    try {
      ...
      const res = await fetch(`https://gitlab.com/api/v4/projects/${projectPath}/repository/branches?per_page=50`, { headers });
      if (!res.ok) {
        return ['main'];
      }
