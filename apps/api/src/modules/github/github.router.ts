    ...
    try {
      ...
      if (existing) {
        throw new AppError(
          409,
          "ALREADY_EXISTS",
          `Repository ${fullName} is already connected`,
        );
      }
...
githubRouter.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return res.json({
        success: true,
        message: `Repository ${deleted.fullName} unlinked`,
      });
    } catch (error) {
      next(error);
    }
      ...
      if (!ghRes || !ghRes.ok) {
        ...
        if (localPrs.length > 0) {
          ...
          return res.json({
            pulls: fallbackPulls,
            warning:
              "GitHub token expired or lacks repo access. Hiển thị PR đã lưu trong hệ thống.",
            isFallback: true,
          });
        }
        ...
        if (ghRes?.status === 404 || (!ghRes && repo.isPrivate)) {
          return res
            ...
            .json({
              pulls: [],
              error:
                "Repository not found on GitHub or private repository requires re-connecting GitHub",
              code: "ACCESS_DENIED",
            });
        }
        return res
          ...
          .json({
            pulls: [],
            error: "GitHub token expired or lacks repo access",
            code: "TOKEN_EXPIRED",
          });
      }
    ...
    try {
      ...
      if (!prUrl) {
        throw new AppError(
          400,
          "BAD_REQUEST",
          "PR url query parameter is required",
        );
      }

      const githubMatch = prUrl.match(
        /github\.com\/([^/]+)\/([^/]+)\/pull\/(\d+)/,
      );
      const gitlabMatch = prUrl.match(
      ...
      if (!githubMatch && !gitlabMatch) {
        throw new AppError(
          400,
          "INVALID_URL",
          "URL is not a valid GitHub pull request or GitLab merge request link",
        );
      }
      ...
      if (!isGitLab && userId) {
        ...
        if (ghRes && ghRes.ok) {
          ...
          if (checksRes.ok) {
            ...
            const hasFailure = checkRunsData.some(
              (r: any) =>
                r.conclusion === "failure" || r.conclusion === "timed_out",
            );
            const hasPending = checkRunsData.some(
              (r: any) => r.status !== "completed",
            );
            const allPass =
              checkRunsData.length > 0 && !hasFailure && !hasPending;
      ...
      if (!pr) {
        ...
        if (liveData.isMerged !== undefined)
          updatePayload.isMerged = liveData.isMerged;
        ...
        if (liveData.authorGithub)
          updatePayload.authorGithub = liveData.authorGithub;
        if (liveData.checksStatus)
          updatePayload.checksStatus = liveData.checksStatus;
        if (liveData.reviewStatus)
          updatePayload.reviewStatus = liveData.reviewStatus;
        if ((liveData as any).rawMetadata)
          updatePayload.rawMetadata = (liveData as any).rawMetadata;
      ...
      return res.json({
        ...
        authorAvatar:
          liveData.authorAvatar ||
          (pr.rawMetadata as any)?.authorAvatar ||
          null,
        checksStatus: pr.checksStatus,
        reviewStatus: pr.reviewStatus,
        rawMetadata: pr.rawMetadata,
        additions:
          liveData.additions ?? (pr.rawMetadata as any)?.additions ?? null,
        deletions:
          liveData.deletions ?? (pr.rawMetadata as any)?.deletions ?? null,
        changedFiles:
          liveData.changedFiles ??
          (pr.rawMetadata as any)?.changedFiles ??
          null,
        url: prUrl,
    ...
    try {
      ...
      if (ghRes && ghRes.ok) {
        ...
        if (hasChanges) {
          ...
          for (const lr of linkedReviews) {
            ...
            const recipientUserIds = [
              ...
              (uid, idx, arr) =>
                uid && uid !== userId && arr.indexOf(uid) === idx,
            );
        ...
        return res.status(200).json({
          success: false,
          error:
            "GitHub token expired or lacks access to this private repository",
          code: "TOKEN_EXPIRED",
          pullRequest: formatPullRequestSummary(pr, repo.fullName),
        });
      }
    ...
    try {
      ...
      const [updated] = await db
        ...
        .set({
          ...
          ...(authorGithub
            ? { authorGithub: String(authorGithub).trim().replace(/^@/, "") }
            : {}),
...
githubRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(
        `GitHub Webhook received: event=${event}, action=${payload?.action}`,
      );
