...
const chunkStorage = multer.diskStorage({
  ...
  filename: (req, _file, cb) => {
    const index =
      req.body.chunkIndex !== undefined ? String(req.body.chunkIndex) : "0";
...
attachmentsRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(
        `[Attachments] Uploaded: ${file.originalname} (${file.size} bytes, ${file.mimetype}) -> id: ${record.id}, target: ${targetType || "none"}:${targetId || "none"}`,
      );
...
attachmentsRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const mimeType =
        (req.body.mimeType as string) || "application/octet-stream";
      const targetType = req.body.targetType as string;
      const targetId = req.body.targetId as string;
      ...
      if (!uploadId || isNaN(chunkIndex) || isNaN(totalChunks)) {
        throw new AppError(
          ...
          "Missing or invalid chunk parameters",
        );
      }
      ...
      if (chunkIndex === totalChunks - 1) {
        ...
        console.log(
          `[Attachments] Chunk upload complete: ${originalName} (${stats.size} bytes) -> id: ${record.id}, target: ${targetType || "none"}:${targetId || "none"}`,
        );
      ...
      console.log(
        `[Attachments] Chunk ${chunkIndex + 1}/${totalChunks} received for upload ${uploadId}`,
      );
    ...
    try {
      ...
      if (isAudio && req.query.download === "true") {
        throw new AppError(
          403,
          "DOWNLOAD_FORBIDDEN",
          "Audio voice recordings cannot be downloaded",
        );
      }
