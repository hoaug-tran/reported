...
issuesRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const repo = issue.repositoryId
        ? await db.query.repositories.findFirst({
            where: eq(repositories.id, issue.repositoryId),
          })
        : null;

      const pr = issue.pullRequestId
        ? await db.query.pullRequests.findFirst({
      ...
      const bugDetails =
        issue.environment || issue.stepsToReproduce
          ? {
              ...
              frequency:
                (issue.frequency as BugFrequency) || BugFrequency.ALWAYS,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (input.labels !== undefined) {
        ...
        const uniqueLabels = [
          ...new Set(
            input.labels.map((l) => l.trim().toLowerCase()).filter(Boolean),
          ),
        ];
