...
export class GoogleAuthProvider implements IAuthProvider {
  ...
  async exchangeCode(code: string): Promise<OAuthTokens> {
    ...
    if (!res.ok) {
      throw new AppError(400, 'OAUTH_TOKEN_EXCHANGE_FAILED', 'Failed to exchange Google authorization code');
    }
    ...
    const scopes = data.scope ? data.scope.split(' ') : ['openid', 'email', 'profile'];
  ...
  async getUserProfile(accessToken: string): Promise<ExternalUserProfile> {
    ...
    if (!userRes.ok) {
      throw new AppError(400, 'OAUTH_USER_PROFILE_FAILED', 'Failed to fetch Google user profile');
    }

    const user = (await userRes.json()) as GoogleUserInfoResponse;
    const username = user.email ? user.email.split('@')[0] : `google_${user.sub.slice(0, 8)}`;
