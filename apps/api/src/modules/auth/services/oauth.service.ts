...
export class OAuthService {
  ...
  verifyState(stateString: string): OAuthStatePayload {
    ...
    if (parts.length !== 2) {
      throw new AppError(
        400,
        "INVALID_OAUTH_STATE",
        "Malformed OAuth state parameter",
      );
    }
    ...
    if (
      ...
      throw new AppError(
        400,
        "INVALID_OAUTH_STATE",
        "Tampered or invalid OAuth state signature",
      );
    }
    ...
    try {
      ...
      if (Date.now() - payload.timestamp > 15 * 60 * 1000) {
        throw new AppError(
          400,
          "EXPIRED_OAUTH_STATE",
          "OAuth authorization state has expired. Please try again.",
        );
      }
  ...
  async processCallback(params: {
    ...
    if (statePayload.intent === "link") {
      ...
      if (!targetUserId) {
        throw new AppError(
          401,
          "UNAUTHORIZED",
          "Target user session required for account linking",
        );
      }
    ...
    let baseUsername =
      profile.username.toLowerCase().replace(/[^a-z0-9_-]/g, "") || "dev_user";
    ...
    const [newUser] = await db
      ...
      .values({
        ...
        email: profile.email
          ...
          : `${chosenUsername}@reported.internal`,
        passwordHash: null,
        displayName: profile.displayName || chosenUsername,
        avatarUrl: profile.avatarUrl,
        bio: profile.bio || null,
        githubUsername:
          providerId.toLowerCase() === "github" ? profile.username : null,
        role: UserRole.USER,
      })
      .returning();
  ...
  async unlinkAccount(userId: string, accountId: string) {
    ...
    return {
      success: true,
      message: `Successfully unlinked ${account.provider} account`,
    };
  }
}
