...
export class PasskeyService {
  ...
  async verifyRegistration(
    ...
    if (!challengeRecord) {
      throw new AppError(
        400,
        "CHALLENGE_EXPIRED",
        "Phiên tạo Passkey đã hết hạn. Vui lòng thử lại.",
      );
    }
    ...
    if (existing) {
      throw new AppError(
        ...
        "Khóa bảo mật (Passkey) này đã được đăng ký.",
      );
    }
    ...
    await db
      .delete(authChallenges)
      .where(eq(authChallenges.id, challengeRecord.id));
  ...
  async verifyLogin(data: { credentialId: string; challengeId?: string }) {
    ...
    if (!passkey) {
      throw new AppError(
        401,
        "PASSKEY_NOT_FOUND",
        "Không tìm thấy thông tin Passkey đã đăng ký.",
      );
    }
    ...
    if (!user) {
      throw new AppError(
        404,
        "USER_NOT_FOUND",
        "Người dùng liên kết với Passkey không tồn tại.",
      );
    }
