...
function verifyPassword(password: string, storedHash?: string | null): boolean {
  ...
  const legacyComputed = crypto
    .scryptSync(password, legacySalt, 64)
    .toString("hex");
...
authRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const extraScopes = req.query.scopes
      ...
      if (
        rawOrigin &&
        (allowedOrigins.includes(rawOrigin) || allowedOrigins.includes("*"))
      ) {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const account = await db.query.externalAccounts.findFirst({
        where: and(
          eq(externalAccounts.id, req.params.id),
    ...
    try {
      ...
      if (!secret || !code) {
        throw new AppError(
          400,
          "INVALID_INPUT",
          "Secret và mã xác thực là bắt buộc",
        );
      }
    ...
    try {
      ...
      if (!tempToken || !code) {
        throw new AppError(
          400,
          "INVALID_INPUT",
          "Thiếu token phiên đăng nhập hoặc mã xác thực",
        );
      }

      let payload: { userId: string; email: string; type: string };
      try {
        payload = jwt.verify(tempToken, config.jwtSecret) as {
          userId: string;
          email: string;
          type: string;
        };
      ...
      if (!isTotpValid) {
        const backupCheck = totpService.verifyBackupCode(
          code,
      ...
      if (!isTotpValid && !isBackupValid) {
        throw new AppError(
          400,
          "INVALID_2FA_CODE",
          "Mã xác thực 2FA hoặc mã dự phòng không đúng.",
        );
      }
...
authRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const created = await passkeyService.verifyRegistration(
        req.user!.id,
        req.body,
      );
