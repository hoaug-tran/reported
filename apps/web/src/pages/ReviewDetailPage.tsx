...
export const ReviewDetailPage: React.FC = () => {
  ...
  const [editReviewType, setEditReviewType] = useState<ReviewType>(
    ReviewType.CODE,
  );
  const [editStatus, setEditStatus] = useState<ReviewStatus>(
  ...
  const fetchReviewData = async (skipPrSync = false) => {
    ...
    try {
      const data = await apiFetch<ReviewDetailDto>(
        `/reviews/${params.number}`,
        { skipCache: true },
      );
      setReview(data);
      setErrorStatus(null);
      ...
      const [commData, actData] = await Promise.all([
        apiFetch<CommentDto[]>(
          `/comments?targetType=REVIEW&targetId=${data.id}`,
        ),
        apiFetch<ActivityTimelineDto[]>(
          `/activity?targetType=REVIEW&targetId=${data.id}`,
        ).catch(() => []),
      ]);
        ...
        apiFetch<{ success: boolean; hasChanges?: boolean }>(
          ...
          .then((res) => {
            if (res?.hasChanges) {
              toast.info(
                isVi
                  ? "Pull Request có cập nhật mới. Đã chuyển trạng thái sang Chờ review."
                  : "Pull Request synced with new changes. Status updated to Pending.",
              );
  ...
  if (!review) {
    ...
    return (
      <NotFoundPage
        message={
          isVi ? "Không tìm thấy yêu cầu review" : "Review request not found"
        }
      />
    );
  }
  ...
  const isAuthor = Boolean(
    user && review?.author?.id && user.id === review.author.id,
  );
  const isReviewer = Boolean(
    user && review?.reviewers?.some((r) => r.user.id === user.id),
  );
  ...
  const handleRemoveReviewer = async (reviewerUserId: string) => {
    ...
    const nextIds = (review.reviewers || [])
      .filter((r) => r.user.id !== reviewerUserId)
      .map((r) => r.user.id);
    try {
      await apiFetch(`/reviews/${review.id}`, {
        method: "PATCH",
      ...
      toast.success(
        isVi ? "Đã xoá reviewer khỏi bài review" : "Reviewer removed",
      );
      fetchReviewData(true);
    } catch (err) {
      toast.error(
        isVi ? "Không thể xoá reviewer" : "Failed to remove reviewer",
      );
    }
  };
    ...
    <Box sx={{ width: "100%" }}>
      ...
      {review.isDeleted && (
        ...
        >
          <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
            ...
            >
              {isVi
                ? "Yêu cầu review này đã bị xoá"
                : "This review request has been deleted"}
            </Typography>
              ...
              >
                ...
                {(review as any).deletedAt && (
                  <>
                    {" "}
                    {isVi ? "lúc" : "at"}{" "}
                    {new Date((review as any).deletedAt).toLocaleString()}
                  </>
                )}
              </Typography>
            )}
        ...
        <Box sx={{ minWidth: 0 }}>
          <Box sx={{ mb: 2 }}>
            ...
            >
              ...
              <Chip
                label={
                  review.reviewType === ReviewType.PR
                    ? "Pull Request (PR)"
                    : review.reviewType === ReviewType.CODE
                      ? "Code Review"
                      : review.reviewType
                }
                sx={{
        ...
        >
          <Box>
            ...
            <Select
              ...
              onChange={(e) =>
                handleStatusChange(e.target.value as ReviewStatus)
              }
              sx={{ fontSize: "0.8125rem" }}
            >
              <MenuItem value={ReviewStatus.PENDING_REVIEW}>
                {isVi ? "Chờ review" : "Pending Review"}
              </MenuItem>
              <MenuItem value={ReviewStatus.IN_REVIEW}>
                {isVi ? "Đang review" : "In Review"}
              </MenuItem>
              <MenuItem value={ReviewStatus.CHANGES_REQUESTED}>
                {isVi ? "Yêu cầu chỉnh sửa" : "Changes Requested"}
              </MenuItem>
              <MenuItem value={ReviewStatus.APPROVED}>
                {isVi ? "Đã duyệt" : "Approved"}
              </MenuItem>
              <MenuItem value={ReviewStatus.CLOSED}>
                {isVi ? "Đã đóng" : "Closed"}
              </MenuItem>
            </Select>
          </Box>
          ...
          <Box>
            ...
            >
              {isVi ? "DANH SÁCH REVIEWER" : "REVIEWERS"} (
            ...
            {!review.reviewers || review.reviewers.length === 0 ? (
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.2 }}>
                {review.reviewers.map((r: ReviewerAssignmentDto) => {
                  ...
                  const isChangesReq =
                    r.status === ReviewerDecision.CHANGES_REQUESTED;
                  ...
                  return (
                    ...
                    >
                      ...
                      {r.acknowledgementStatus &&
                        ackStatusConfig[r.acknowledgementStatus] &&
                        (() => {
      ...
      >
        ...
        >
          ...
          {isVi
            ? `Xóa yêu cầu review #${review.number}?`
            : `Delete review #${review.number}?`}
        </DialogTitle>
        ...
        >
          ...
          <Box>
            ...
            <TextField
              ...
              placeholder={
                isVi ? "Nhập tiêu đề review..." : "Enter review title..."
              }
          ...
          >
            <Box>
              ...
              <Select
                ...
                onChange={(e) =>
                  setEditReviewType(e.target.value as ReviewType)
                }
                sx={{ borderRadius: "6px" }}
              >
                <MenuItem value={ReviewType.CODE}>
                  {isVi ? "Triển khai Code" : "Code Implementation"}
                </MenuItem>
                <MenuItem value={ReviewType.PR}>
                  {isVi ? "Review Pull Request" : "Pull Request Review"}
                </MenuItem>
                <MenuItem value={ReviewType.ARCHITECTURE}>
                  {isVi ? "Kiến trúc & Thiết kế" : "Architecture & Design"}
                </MenuItem>
                <MenuItem value={ReviewType.SECURITY}>
                  {isVi ? "Bảo mật" : "Security Review"}
                </MenuItem>
                <MenuItem value={ReviewType.API}>
                  {isVi ? "API & Tích hợp" : "API & Integration"}
                </MenuItem>
                <MenuItem value={ReviewType.DATABASE}>
                  {isVi ? "Cơ sở dữ liệu" : "Database & Schema"}
                </MenuItem>
                <MenuItem value={ReviewType.UI}>
                  {isVi ? "Giao diện & Frontend" : "UI & Frontend"}
                </MenuItem>
          ...
          <Box>
            ...
            <MarkdownEditor
              ...
              placeholder={
                isVi
                  ? "Mô tả chi tiết những gì cần review..."
