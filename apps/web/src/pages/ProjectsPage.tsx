...
export const ProjectsPage: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        isVi ? "Tạo dự án mới thành công!" : "Project created successfully!",
      );
      ...
      const errMsg =
        err instanceof Error
          ...
          : (err as { message?: string })?.message ||
            (isVi ? "Lỗi khi tạo dự án" : "Error creating project");
      toast.error(errMsg);
    } finally {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (!editName.trim() || !editKey.trim() || !editSlug.trim()) {
      toast.error(
        isVi
          ? "Tên, slug và mã dự án không được để trống"
          : "Name, slug, and key cannot be empty",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        isVi ? "Đã cập nhật dự án thành công" : "Project updated successfully",
      );
      ...
      const errMsg =
        err instanceof Error
          ? err.message
          : (err as { message?: string })?.message ||
            (isVi ? "Lỗi khi cập nhật dự án" : "Error updating project");
      toast.error(errMsg);
    } finally {
      setEditSaving(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        isVi ? "Đã xóa dự án thành công" : "Project deleted successfully",
      );
      ...
      const errMsg =
        err instanceof Error
          ? err.message
          : (err as { message?: string })?.message ||
            (isVi ? "Lỗi khi xóa dự án" : "Error deleting project");
      toast.error(errMsg);
    } finally {
    ...
    <Box sx={{ width: "100%", pb: 8 }}>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ mb: 1.5 }}>
          ...
          >
            Phân chia các thành phần kiến trúc, microservices và frontend theo
            dự án chuyên biệt
          </Typography>
        </Box>
          ...
          <Grid item xs={12}>
            ...
            >
              <Typography variant="body1" sx={{ color: tokens.textSecondary }}>
                Chưa có dự án nào trong không gian này. Hãy bấm "Thêm dự án" để
                bắt đầu.
              </Typography>
            </Paper>
          </Grid>
                ...
                >
                  <CardContent sx={{ p: 0 }}>
                    ...
                    >
                      {proj.description ||
                        (isVi
                          ? "Dự án kỹ thuật thuộc không gian làm việc."
                          : "Technical project in this workspace.")}
                    </Typography>
        ...
        <form onSubmit={handleCreate}>
          ...
          >
            ...
            <TextField
              ...
              placeholder={
                isVi
                  ? "Mô tả phạm vi, công nghệ sử dụng và mục tiêu..."
                  : "Scope, technologies, and objectives..."
              }
              value={description}
      ...
      <Dialog
        ...
        onClose={() => !deleteDeleting && setDeleteDialogOpen(false)}
        maxWidth="xs"
        fullWidth
        PaperProps={{ sx: { borderRadius: "8px" } }}
      >
