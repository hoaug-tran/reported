...
export const DashboardPage: React.FC = () => {
    ...
    async function loadDashboardData() {
      ...
      try {
        const [
          ...
          user
            ? apiFetch<{ data: ReviewDetailDto[] }>(
                `/reviews?reviewerId=${user.id}&limit=10`,
              )
            : Promise.resolve({ data: [] }),
          user
            ? apiFetch<{ notifications: NotificationDto[] }>(
                "/notifications?unreadOnly=true",
              )
            : Promise.resolve({ notifications: [] }),
          apiFetch<{ data: IssueDto[] }>(
            "/issues?priority=P0&priority=P1&status=OPEN&status=IN_PROGRESS&limit=5",
          ),
          user
            ? apiFetch<{ data: ReviewDetailDto[] }>(
                `/reviews?authorId=${user.id}&limit=10`,
              )
            : Promise.resolve({ data: [] }),
          apiFetch<{ data: IssueDto[] }>("/issues?sortBy=updated&limit=15"),
          apiFetch<{ data: ReviewDetailDto[] }>(
            "/reviews?sortBy=updated&limit=15",
          ),
        ...
        const pendingForMe = (reviewsToMeRes.data || []).filter(
          (r) =>
            r.status === ReviewStatus.PENDING_REVIEW ||
        ...
        const issueItems: ConversationItem[] = (recentIssuesRes.data || []).map(
          (iss) => ({
            id: iss.id,
            kind: "issue",
            type: iss.type,
        ...
        const reviewItems: ConversationItem[] = (
  ...
  const filteredConversations = conversations.filter((item) => {
    ...
    if (feedFilter === "bugs")
      return item.type === IssueType.BUG || item.type === "BUG";
    if (feedFilter === "reviews") return item.kind === "review";
    if (feedFilter === "questions")
      return item.type === IssueType.TASK || item.type === "QUESTION";
    return true;
  });

  const totalAttentionItems =
    pendingReviews.length + mentions.length + urgentIssues.length;
          ...
          <Box>
            ...
            {totalAttentionItems === 0 ? (
              ...
              >
                ...
                >
                  {isVi
                    ? "Bạn đã hoàn tất mọi việc cần xử lý!"
            ...
            {filteredConversations.length === 0 ? (
              ...
              >
                {filteredConversations.map((item, idx) => {
                  const isBug =
                    item.type === IssueType.BUG || item.type === "BUG";
