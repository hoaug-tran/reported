...
function GitLabIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    ...
    >
      <path
        d="m23.6 9.59-1.26-3.87a.78.78 0 0 0-1.48 0L19.6 9.59H4.4L3.14 5.72a.78.78 0 0 0-1.48 0L.4 9.59a.78.78 0 0 0 .28.87l11.07 8.05a.78.78 0 0 0 .9 0l11.07-8.05a.78.78 0 0 0 .28-.87z"
        fill="#E24329"
      />
...
export const ConnectedAccountsPage: React.FC = () => {
  ...
  const [unlinkConfirmTarget, setUnlinkConfirmTarget] =
    useState<ConnectedAccountDto | null>(null);
      ...
      const errMsg =
        err instanceof Error
          ...
          : (err as { message?: string })?.message ||
            `Không thể bắt đầu kết nối ${provider}`;
  ...
  const handleExecuteUnlink = async () => {
    ...
    try {
      ...
      setActionSuccess(
        `Đã hủy liên kết tài khoản ${unlinkConfirmTarget.provider.toUpperCase()} thành công.`,
      );
  ...
  const handleReconnect = async (acc: ConnectedAccountDto) => {
    ...
    try {
      ...
      setActionSuccess(
        `Đã làm mới trạng thái kết nối ${acc.provider.toUpperCase()} thành công.`,
      );
    } catch (err: unknown) {
      const errMsg =
        err instanceof Error
          ? err.message
          : (err as { message?: string })?.message || "Không thể kết nối lại.";
      setActionError(errMsg);
    } finally {
      setOperatingId(null);
    }
  ...
  const handleSavePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setConfirmPassword("");
      setActionSuccess(
        "Đã thiết lập mật khẩu đăng nhập thành công. Bạn có thể đăng nhập bằng email và mật khẩu này.",
      );
    } catch (err: unknown) {
      const errMsg =
        err instanceof Error
  ...
  const handleRegisterPasskey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rawChallenge = options.challenge
        .replace(/-/g, "+")
        .replace(/_/g, "/");
      const challengeBytes = Uint8Array.from(atob(rawChallenge), (c) =>
        c.charCodeAt(0),
      );

      const rawUserId = options.user.id.replace(/-/g, "+").replace(/_/g, "/");
      const userIdBytes = Uint8Array.from(atob(rawUserId), (c) =>
        c.charCodeAt(0),
      );
      ...
      setActionSuccess(
        `Đã đăng ký thành công khóa bảo mật "${passkeyName}". Bạn có thể đăng nhập bằng 1 chạm.`,
      );
      await fetchPasskeys();
    } catch (err: unknown) {
      const errMsg =
  ...
  const handleDeletePasskey = async (id: string, name: string) => {
    ...
    try {
      ...
      const errMsg =
        err instanceof Error ? err.message : "Không thể xóa khóa Passkey";
      setActionError(errMsg);
    }
  };
  ...
  const handleStartMfaSetup = async () => {
    ...
    try {
      ...
      const errMsg =
        err instanceof Error ? err.message : "Không thể tạo cấu hình 2FA";
      setActionError(errMsg);
    }
  };
  ...
  const handleEnableMfa = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActionSuccess(
        "Xác thực hai yếu tố (2FA) đã được kích hoạt thành công!",
      );
      await refreshUser();
    } catch (err: unknown) {
      const errMsg =
  ...
  const providersList = [
    {
      ...
      name: "GitHub",
      description:
        "Đồng bộ Repository, Pull Request, Commit và Branch trực tiếp vào Issue & Review.",
      icon: <GitHubIcon />,
      color: "#24292f",
    },
    {
      ...
      name: "GitLab",
      description:
        "Hỗ trợ GitLab Projects và Merge Requests (MR) cho các nhóm DevOps.",
      icon: <GitLabIcon />,
      color: "#FC6D26",
    },
    {
      ...
      name: "Google",
      description:
        "Đăng nhập bảo mật nhanh bằng tài khoản Google Workspace hoặc Gmail cá nhân.",
      icon: <GoogleIcon />,
      color: "#4285F4",
    },
  ];
  ...
  return (
    <Box sx={{ width: "100%", pb: 6 }}>
      ...
      {!user?.hasPassword && connectedAccounts.length === 1 && (
        ...
        >
          <strong>Cảnh báo khóa tài khoản:</strong> Bạn hiện chỉ có duy nhất
          phương thức đăng nhập qua{" "}
          <strong>{connectedAccounts[0].provider.toUpperCase()}</strong> và chưa
          có mật khẩu ứng dụng. Hãy thiết lập mật khẩu để không bị mất quyền
          truy cập nếu hủy liên kết tài khoản này.
        </Alert>
      )}
                    ...
                    {connected && (
                      ...
                      >
                        ...
                        {connected.scopes && connected.scopes.length > 0 && (
                          <Tooltip
                            title={`Quyền đã cấp: ${connected.scopes.join(", ")}`}
                          >
                            <Chip
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  ...
                  <Box>
                    ...
                    >
                      ...
                      {pk.lastUsedAt &&
                        ` • Dùng gần nhất: ${new Date(pk.lastUsedAt).toLocaleDateString()}`}
                    </Typography>
                  </Box>
                </Box>
      ...
      >
        <Box component="form" onSubmit={handleEnableMfa}>
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: tokens.textSecondary }}>
              1. Mở ứng dụng <strong>Google Authenticator</strong> hoặc{" "}
              <strong>Authy</strong> trên điện thoại.
            </Typography>
            ...
            <Box>
              <Typography
                variant="body2"
                sx={{ color: tokens.textSecondary, mb: 1 }}
              >
                3. Nhập mã 6 chữ số hiển thị trên ứng dụng Authenticator để hoàn
                tất kích hoạt:
              </Typography>
              <TextField
                fullWidth
          ...
          <DialogActions sx={{ p: 2 }}>
            ...
            <Button
              type="submit"
              variant="contained"
              disabled={mfaSubmitting || mfaVerifyCode.trim().length !== 6}
            >
      ...
      <Dialog
        open={Boolean(unlinkConfirmTarget)}
        onClose={() => setUnlinkConfirmTarget(null)}
        maxWidth="xs"
        fullWidth
      >
        <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 2 }}>
            ...
            <strong>{unlinkConfirmTarget?.provider.toUpperCase()}</strong> ( @
            {unlinkConfirmTarget?.username || unlinkConfirmTarget?.email})
            không?
          </Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          ...
          >
            {operatingId === unlinkConfirmTarget?.id
              ? "Đang xử lý..."
