...
export const MembersPage: React.FC = () => {
  ...
  const [removeTarget, setRemoveTarget] = useState<WorkspaceMemberDto | null>(
    null,
  );
  ...
  const fetchMembers = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await apiFetch<WorkspaceMemberDto[]>(
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      await apiFetch(
        `/workspaces/${activeWorkspace.id}/members/${removeTarget.userId}`,
        {
          method: "DELETE",
        },
      );
                      ...
                      <TableCell sx={{ whiteSpace: "nowrap" }}>
                        <Select
                          ...
                          disabled={
                            ...
                            (isOwner &&
                              members.filter(
                                (item) => item.role === WorkspaceRole.OWNER,
                              ).length <= 1)
                          }
                          sx={{
                        ...
                        >
                          <MenuItem value={WorkspaceRole.OWNER}>
                            {t("roleOwner")}
                          </MenuItem>
                          ...
                          <MenuItem value={WorkspaceRole.GUEST}>
                            {t("roleGuest")}
                          </MenuItem>
                        </Select>
                      </TableCell>
        ...
        <form onSubmit={handleInvite}>
          ...
          >
            ...
            <FormControl fullWidth>
              ...
              >
                <MenuItem value={WorkspaceRole.ADMIN}>
                  {t("roleAdmin")} (Toàn quyền quản lý)
                </MenuItem>
                <MenuItem value={WorkspaceRole.MEMBER}>
                  {t("roleMember")} (Tạo issue, gửi review)
                </MenuItem>
                <MenuItem value={WorkspaceRole.GUEST}>
                  {t("roleGuest")} (Chỉ xem và bình luận)
                </MenuItem>
              </Select>
            </FormControl>
          </DialogContent>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ color: "#8b949e" }}>
            Bạn có chắc muốn xóa <strong>{removeTarget?.displayName}</strong> (@
            {removeTarget?.username}) khỏi không gian làm việc?
          </Typography>
          <Typography
            variant="caption"
            sx={{ display: "block", mt: 1, color: "#8b949e" }}
          >
            Thành viên sẽ mất quyền truy cập ngay lập tức. Bạn có thể mời lại họ
            sau.
          </Typography>
        </DialogContent>
