...
export const PullRequestPreview: React.FC<PullRequestPreviewProps> = ({
  ...
  const [editAdditions, setEditAdditions] = useState(
    String(initialPr.additions ?? ""),
  );
  const [editDeletions, setEditDeletions] = useState(
    String(initialPr.deletions ?? ""),
  );
  const [editFiles, setEditFiles] = useState(
    String(initialPr.changedFiles ?? ""),
  );
  ...
  const additionsRatio =
    totalDiff > 0 ? Math.round(((pr.additions || 0) / totalDiff) * 100) : 50;
    ...
    try {
      ...
      if (res.success) {
        setSyncFeedback({
          type: "success",
          message: isVi
            ? "Đồng bộ dữ liệu Pull Request từ GitHub thành công!"
            : "PR data successfully synced from GitHub!",
        });
      ...
      setSyncFeedback({
        ...
        message:
          err.message ||
          (isVi ? "Lỗi kết nối khi đồng bộ PR" : "Error syncing PR"),
      });
    } finally {
      setIsSyncing(false);
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setSyncFeedback({
        type: "success",
        message: isVi
          ? "Cập nhật thông tin PR thành công!"
          : "PR details updated successfully!",
      });
      if (onRefresh) onRefresh();
    } catch (err: any) {
      setSyncFeedback({
        ...
        message:
          err.message ||
          (isVi ? "Không thể cập nhật PR" : "Failed to update PR"),
      });
    } finally {
      setIsSaving(false);
    }
    ...
    switch (pr.checksStatus) {
      case PullRequestChecksStatus.PASSING:
        return (
          <Tooltip
            title={
              isVi ? "Tất cả CI checks đều passed" : "All CI checks passed"
            }
          >
            <Box
              sx={{
      ...
      case PullRequestChecksStatus.PENDING:
        return (
          <Tooltip
            title={isVi ? "CI checks đang thực hiện" : "CI checks pending"}
          >
            <Box
              sx={{
                display: "inline-flex",
      ...
      case PullRequestChecksStatus.FAILING:
        return (
          <Tooltip
            title={isVi ? "Một số CI check bị lỗi" : "Some CI checks failed"}
          >
            <Box
              sx={{
                display: "inline-flex",
    ...
    >
      ...
      >
        ...
        >
          <Tooltip
            title={
              isVi
                ? "Đồng bộ dữ liệu trực tiếp từ GitHub"
                : "Sync fresh data directly from GitHub"
            }
          >
            <span>
              <Button
          ...
          <Tooltip
            title={
              isVi ? "Sửa thông tin PR thủ công" : "Edit PR details manually"
            }
          >
            <IconButton
              size="small"
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
            ...
            >
              <Box
                ...
                onClick={() =>
                  githubProfileUrl && window.open(githubProfileUrl, "_blank")
                }
              >
                {pr.authorAvatar ? (
                  <Avatar
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              <Chip
                ...
                sx={{
                  ...
                  borderColor: tokens.border,
                  backgroundColor:
                    resolvedMode === "dark" ? "#0d1117" : "#f8fafc",
                }}
              />
            </Box>
          )}
          ...
          {(pr.additions != null ||
            ...
            <Box
              sx={{ display: "flex", alignItems: "center", gap: 1.2, ml: 0.5 }}
            >
              {pr.changedFiles != null && (
                <Box
                  sx={{
                ...
                >
                  <FileCode size={13} />
                  <span>
                    {pr.changedFiles} {isVi ? "tệp đổi" : "files"}
                  </span>
                </Box>
              )}
      ...
      {checkRunsCount > 0 && (
        <Collapse in={showCheckRuns}>
          <Box
            sx={{ mt: 1.5, pt: 1.5, borderTop: `1px dashed ${tokens.border}` }}
          >
            <Box
            ...
            >
              ...
              >
                {isVi
                  ? "CHI TIẾT CÁC BƯỚC KIỂM TRA CI/CD"
                  : "CI/CD CHECK RUNS DETAILS"}
              </Typography>
            </Box>
            <CICheckRunsList
      ...
      >
        ...
        >
          {isVi
            ? "Chỉnh sửa thông tin Pull Request liên kết"
            : "Edit Linked Pull Request"}
        </DialogTitle>
        <DialogContent
        ...
        >
          ...
          <TextField
            label={
              isVi
                ? "Tác giả PR trên GitHub (username)"
            ...
            helperText={
              isVi
                ? "Ví dụ: hoaug-tran hoặc tên lập trình viên mở PR trên GitHub"
                : "e.g. hoaug-tran or collaborator GitHub username"
            }
            value={editAuthor}
