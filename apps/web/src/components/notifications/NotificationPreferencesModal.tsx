...
const CHANNEL_LABELS: Record<NotificationChannel, { vi: string; en: string }> = {
  [NotificationChannel.IN_APP]: { vi: 'Chỉ trong ứng dụng', en: 'In-App Only' },
  [NotificationChannel.EMAIL]: { vi: 'Chỉ qua Email', en: 'Email Only' },
  [NotificationChannel.BOTH]: { vi: 'Cả ứng dụng & Email', en: 'Both (App & Email)' },
...
export const NotificationPreferencesModal: React.FC<NotificationPreferencesModalProps> = ({ open, onClose }) => {
  ...
  const [desktopPermission, setDesktopPermission] = useState<string>(getDesktopNotificationPermission());
  ...
  const handleTestDesktopNotification = () => {
    showDesktopNotification(
      ...
      {
        body: isVi ? 'Hệ thống thông báo đẩy trên máy tính đang hoạt động chính xác!' : 'Desktop push notifications are working properly!'
      }
    );
  };
  ...
  return (
    ...
    >
      <DialogTitle sx={{ fontWeight: 700, borderBottom: `1px solid ${tokens.border}`, pb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
        <Bell size={20} color={tokens.primary} />
        {isVi ? 'Cài đặt Thông báo & Kênh gửi tin' : 'Notification & Email Preferences'}
      </DialogTitle>
      ...
      <DialogContent sx={{ pt: 2.5 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              <Typography variant="body2" sx={{ fontWeight: 600 }}>
                {isVi ? 'Thông báo màn hình máy tính (Desktop Notification)' : 'Desktop Push Notifications'}
              </Typography>
              <Typography variant="caption" sx={{ color: tokens.textSecondary }}>
                {desktopPermission === 'granted'
                  ? (isVi ? 'Đã cho phép thông báo màn hình khi có cập nhật mới' : 'Permission granted for desktop alerts')
                  : (isVi ? 'Chưa bật quyền nhận thông báo trên máy tính' : 'Permission not yet granted')}
              </Typography>
            </Box>
          </Box>
            ...
            {desktopPermission !== 'granted' ? (
              ...
              <>
                <Chip
                  ...
                  size="small"
                  sx={{ backgroundColor: 'rgba(63, 185, 80, 0.15)', color: tokens.success, height: 24, fontSize: '0.6875rem', fontWeight: 600 }}
                />
                <Button
                  size="small"
                  variant="outlined"
        ...
        {savedSuccess && (
          <Alert severity="success" sx={{ mb: 2, borderRadius: '6px' }}>
            {isVi ? 'Đã lưu cài đặt thông báo thành công!' : 'Notification preferences saved successfully!'}
          </Alert>
        )}
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 700, color: tokens.textPrimary, borderBottom: `1px solid ${tokens.border}` }}>
                {isVi ? 'Sự kiện kích hoạt' : 'Trigger Event'}
              </TableCell>
              <TableCell sx={{ fontWeight: 700, color: tokens.textPrimary, width: 200, borderBottom: `1px solid ${tokens.border}` }}>
          ...
          <TableBody>
            {Object.entries(EVENT_LABELS).map(([eventType, labelObj]) => (
              <TableRow key={eventType}>
                <TableCell sx={{ fontSize: '0.84rem', py: 1.2, borderBottom: `1px solid ${tokens.border}` }}>
                  {isVi ? labelObj.vi : labelObj.en}
                </TableCell>
                <TableCell sx={{ py: 1.2, borderBottom: `1px solid ${tokens.border}` }}>
                  <Select
                    size="small"
                    fullWidth
                  ...
                  >
                    <MenuItem value={NotificationChannel.IN_APP}>{CHANNEL_LABELS[NotificationChannel.IN_APP][isVi ? 'vi' : 'en']}</MenuItem>
                    <MenuItem value={NotificationChannel.EMAIL}>{CHANNEL_LABELS[NotificationChannel.EMAIL][isVi ? 'vi' : 'en']}</MenuItem>
                    <MenuItem value={NotificationChannel.BOTH}>{CHANNEL_LABELS[NotificationChannel.BOTH][isVi ? 'vi' : 'en']}</MenuItem>
                    <MenuItem value={NotificationChannel.DISABLED}>{CHANNEL_LABELS[NotificationChannel.DISABLED][isVi ? 'vi' : 'en']}</MenuItem>
                  </Select>
                </TableCell>
              </TableRow>
            ))}
      ...
      <DialogActions sx={{ p: 2, borderTop: `1px solid ${tokens.border}` }}>
        <Button onClick={onClose} color="inherit" sx={{ textTransform: 'none', borderRadius: '6px' }}>
          {isVi ? 'Hủy' : 'Cancel'}
        </Button>
        <Button onClick={handleSave} variant="contained" disabled={isSaving} sx={{ textTransform: 'none', borderRadius: '6px' }}>
          {isSaving ? (isVi ? 'Đang lưu...' : 'Saving...') : (isVi ? 'Lưu thay đổi' : 'Save Preferences')}
        </Button>
      </DialogActions>
    </Dialog>
  );
