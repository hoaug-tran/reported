...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await apiFetch<SearchResultItemDto[]>(
  ...
  const defaultActions = [
    ...
    },
    {
      id: "toggle-theme",
      title: isVi
        ? `Chuyển đổi giao diện sáng/tối (Hiện tại: ${mode})`
  ...
  > = {
    ISSUE: {
      label: isVi ? "Vấn đề" : "Issue",
      color: "#f85149",
      bg: "rgba(248, 81, 73, 0.15)",
    },
    REVIEW: {
      label: isVi ? "Review" : "Review",
      color: "#a371f7",
    ...
    COMMENT: {
      label: isVi ? "Bình luận" : "Comment",
      color: "#10b981",
      bg: "rgba(16, 185, 129, 0.15)",
    },
    USER: {
      label: isVi ? "Thành viên" : "User",
      color: "#eab308",
      bg: "rgba(234, 179, 8, 0.15)",
    },
    REPOSITORY: {
      label: isVi ? "Kho lưu trữ" : "Repo",
      color: "#6366f1",
      ...
      <DialogContent sx={{ p: 0 }}>
        <Box sx={{ p: 1.5, borderBottom: `1px solid ${tokens.border}` }}>
          <TextField
            ...
            placeholder={
              isVi
                ? "Tìm kiếm theo #mã, tên vấn đề, review, dự án, bình luận..."
                : "Type a command or search #number, issues, reviews, projects, comments..."
            }
            variant="standard"
            InputProps={{
              ...
              {results.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <ListItemIcon sx={{ minWidth: 32 }}>
                      {item.type === "ISSUE" && (
                        <Bug size={18} color={tokens.primary} />
                      )}
                      {item.type === "REVIEW" && (
                      ...
                      {item.type === "USER" && (
                        <User size={18} color={tokens.textSecondary} />
                      )}
                      {item.type === "REPOSITORY" && (
                        <FolderGit2 size={18} color={tokens.textSecondary} />
                      )}
                    </ListItemIcon>
                    <ListItemText
                      primary={
