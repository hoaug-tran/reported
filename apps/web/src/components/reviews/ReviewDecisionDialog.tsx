...
import { CheckCircle2, AlertCircle, MessageSquare, ShieldCheck, Info } from 'lucide-react';
  ...
  const options = [
    {
      ...
      desc: isVi ? 'Đồng ý với các thay đổi và cho phép merge/triển khai code.' : 'Submit feedback and approve merging or implementation.',
      icon: CheckCircle2,
    ...
    },
    {
      ...
      desc: isVi ? 'Yêu cầu tác giả phải sửa các lỗi/vấn đề trước khi được duyệt.' : 'Submit feedback that must be addressed before approval.',
      icon: AlertCircle,
      color: tokens.error,
    ...
    },
    {
      ...
      desc: isVi ? 'Chỉ nhận xét/góp ý thông thường mà không chặn/chấp thuận.' : 'Submit general feedback without explicit approval or rejection.',
      icon: MessageSquare,
    ...
    <Dialog
      ...
      PaperProps={{
        sx: {
          ...
          boxShadow: isDark ? '0 16px 36px rgba(0,0,0,0.5)' : '0 10px 25px rgba(0,0,0,0.1)',
          overflow: 'hidden'
        }
      }}
    >
      <DialogTitle sx={{ p: 2.5, pb: 2, borderBottom: `1px solid ${tokens.borderSubtle}` }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
          <Box>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, fontSize: '0.98rem', color: tokens.textPrimary, lineHeight: 1.2 }}>
              {isVi ? 'Gửi quyết định Review' : 'Submit Review Decision'}
            </Typography>
            <Typography variant="caption" sx={{ color: tokens.textSecondary, fontSize: '0.76rem' }}>
              {isVi
      ...
      <DialogContent sx={{ p: 2.5, pt: '20px !important' }}>
        {error && (
          <Alert severity="error" sx={{ mb: 2, borderRadius: '6px', py: 0.5 }} onClose={() => setError(null)}>
            {error}
          </Alert>
        )}

        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, mt: 1.5, mb: 2.5 }}>
          {options.map((opt) => {
            ...
            return (
              <Paper
                ...
                sx={{
                  ...
                  backgroundColor: isSelected ? opt.activeBg : tokens.surfaceSecondary,
        ...
        <TextField
          ...
          multiline
          rows={2.5}
          label={isVi ? 'Ghi chú quyết định (Không bắt buộc)' : 'Decision Note (Optional)'}
          placeholder={
            isVi
        ...
        <Box
          sx={{
            ...
            px: 1.25,
            borderRadius: '6px',
            backgroundColor: isDark ? 'rgba(56, 139, 253, 0.08)' : 'rgba(9, 105, 218, 0.05)',
            border: `1px solid ${isDark ? 'rgba(56, 139, 253, 0.15)' : 'rgba(9, 105, 218, 0.12)'}`
          }}
        >
          <Info size={14} color={tokens.primary} style={{ flexShrink: 0 }} />
          <Typography variant="caption" sx={{ color: tokens.textSecondary, fontSize: '0.73rem', lineHeight: 1.35 }}>
            {isVi
      ...
      <DialogActions sx={{ px: 2.5, py: 2, borderTop: `1px solid ${tokens.borderSubtle}`, gap: 1.25 }}>
        <Button
          onClick={onClose}
          color="inherit"
        ...
        >
          {isSubmitting
            ...
            : decision === ReviewerDecision.APPROVED
            ? (isVi ? 'Chấp thuận Review' : 'Approve Review')
            : decision === ReviewerDecision.CHANGES_REQUESTED
            ? (isVi ? 'Yêu cầu sửa đổi' : 'Request Changes')
            : (isVi ? 'Gửi nhận xét' : 'Submit Comment')}
        </Button>
      </DialogActions>
    </Dialog>
  );
