...
import { ReviewType, CreateReviewDto, RepositoryDto, UserSummaryDto } from '@reported/contracts';
...
export const CreateReviewModal: React.FC<CreateReviewModalProps> = ({ open, onClose, onSuccess }) => {
  ...
  const preserveDraft = () => {
    sessionStorage.setItem('reported_review_draft', JSON.stringify({
      title,
      description,
      reviewType,
      prUrl,
  ...
  const handleSubmit = async () => {
    ...
    if (selectedReviewerIds.length === 0) {
      setError(isVi ? 'Vui lòng chọn ít nhất một reviewer' : 'Please assign at least one reviewer');
      return;
    }
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : (err as { message?: string })?.message || 'Failed to create review request';
      setError(errMsg);
    } finally {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ borderBottom: `1px solid ${tokens.border}`, pb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      <DialogContent sx={{ pt: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <TextField
          ...
          placeholder={isVi ? 'Ví dụ: Auth refresh token rotation & reuse detection' : 'e.g. Auth refresh token rotation & reuse detection'}
          value={title}
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
          <FormControl size="small" fullWidth>
            ...
            <Select
              ...
              sx={inputSx(tokens)}
            >
              <MenuItem value={ReviewType.CODE}>{isVi ? 'Code Review' : 'Code Review'}</MenuItem>
              <MenuItem value={ReviewType.ARCHITECTURE}>{isVi ? 'Kiến trúc RFC' : 'Architecture RFC'}</MenuItem>
              <MenuItem value={ReviewType.DATABASE}>{isVi ? 'Schema Database' : 'Database Schema'}</MenuItem>
              <MenuItem value={ReviewType.API}>{isVi ? 'Thiết kế API' : 'API Design'}</MenuItem>
              <MenuItem value={ReviewType.SECURITY}>{isVi ? 'Bảo mật' : 'Security'}</MenuItem>
              <MenuItem value={ReviewType.UI}>{isVi ? 'UI / UX' : 'UI / UX'}</MenuItem>
              <MenuItem value={ReviewType.DOCUMENTATION}>{isVi ? 'Tài liệu kỹ thuật' : 'Documentation'}</MenuItem>
              <MenuItem value={ReviewType.PR}>{isVi ? 'Pull Request' : 'Pull Request'}</MenuItem>
            </Select>
          </FormControl>
        ...
        <Box>
          <Typography variant="caption" sx={{ color: tokens.textSecondary, mb: 0.5, display: 'block', fontWeight: 600 }}>
            {isVi ? 'BỐI CẢNH & HƯỚNG DẪN REVIEW (Markdown)' : 'OBJECTIVES & CONTEXT (Markdown)'}
          </Typography>
          <MarkdownEditor
            value={description}
        ...
        {!hasCodeHostingConnected && (
          <ContextualCodeHostingNotice onPreserveDraft={preserveDraft} returnPath="/reviews" />
        )}
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
          ...
          <TextField
            size="small"
            fullWidth
            label={isVi ? 'Link PR GitHub (tùy chọn)' : 'GitHub PR URL (optional)'}
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
          <Autocomplete
            ...
            onChange={(_, vals) => setSelectedReviewerIds(vals.map(v => v.id))}
            renderInput={(params) => (
              <TextField
                {...params}
                label={isVi ? 'Chỉ định Reviewer *' : 'Reviewers *'}
                placeholder={isVi ? 'Gõ để tìm đồng nghiệp...' : 'Assign reviewers...'}
                sx={inputSx(tokens)}
              />
            )}
          />
          ...
          <Autocomplete
            ...
            freeSolo
            size="small"
            options={['architecture', 'security', 'database', 'api', 'perf', 'frontend', 'review']}
            value={labels}
      ...
      <DialogActions sx={{ p: 2, borderTop: `1px solid ${tokens.border}`, justifyContent: 'space-between' }}>
        ...
        <Button
          ...
          variant="contained"
          disabled={isSubmitting || !title.trim() || selectedReviewerIds.length === 0}
          sx={buttonSx(tokens)}
        >
          {isSubmitting ? (isVi ? 'Đang gửi...' : 'Submitting...') : (isVi ? 'Gửi yêu cầu Review' : 'Request Review')}
        </Button>
      </DialogActions>
    </Dialog>
  );
