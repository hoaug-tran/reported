...
export const JsonViewer: React.FC<JsonViewerProps> = ({
  ...
  const renderValue = (
    ...
    if (typeof value === "boolean")
      return (
        <span style={{ color: "#ff7b72" }}>{value ? "true" : "false"}</span>
      );
    ...
    const isCollapsed =
      collapsedKeys.has(path) ||
      (depth >= initialCollapsedDepth && !collapsedKeys.has(`open_${path}`));
    ...
    return (
      <Box component="span" sx={{ display: "inline-block" }}>
        ...
        <Collapse in={!isCollapsed}>
          <Box
            sx={{ pl: 2, borderLeft: `1px dashed ${tokens.border}`, my: 0.25 }}
          >
            {keys.map((k) => {
              ...
              return (
                <Box key={k} sx={{ lineHeight: 1.5 }}>
                  <span
                    style={{
                      color: resolvedMode === "dark" ? "#7ee787" : "#116329",
                      fontWeight: 500,
                    }}
                  >
                    "{k}"
                  </span>
                  <span style={{ color: tokens.textSecondary, marginRight: 6 }}>
                    :{" "}
                  </span>
