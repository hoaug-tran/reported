...
export function useWeatherAndClock(isVi = true) {
        ...
        if (!detectedLocation) {
          try {
            const geoRes = await fetch(
              `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lon}&localityLanguage=${isVi ? "vi" : "en"}`,
            );
            if (geoRes.ok) {
  ...
  const dayOfWeek = isVi
    ? dayOfWeekNamesVi[now.getDay()]
    : dayOfWeekNamesEn[now.getDay()];
