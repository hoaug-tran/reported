...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithPasskey = async () => {
    if (!window.PublicKeyCredential) {
      throw new Error(
        "Thiết bị hoặc trình duyệt không hỗ trợ xác thực Passkey.",
      );
    }
  ...
  const reconnectAccount = async (id: string) => {
    await apiFetch(`/auth/connected-accounts/${id}/reconnect`, {
      method: "POST",
    });
    await fetchConnectedAccounts();
  };
  ...
  const hasCodeHostingConnected = Boolean(
    ...
    connectedAccounts.some(
      (acc) =>
        (acc.provider === "github" || acc.provider === "gitlab") &&
        acc.healthStatus !== "REVOKED",
    ),
  );
