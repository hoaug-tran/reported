...
import { ReviewerDecision, ReviewStatus, ReviewType } from '@reported/contracts';
...
export const reviewReviewers = pgTable('review_reviewers', {
  reviewId: uuid('review_id').notNull().references(() => reviewRequests.id, { onDelete: 'cascade' }),
...
export const reviewLabels = pgTable('review_labels', {
  ...
  primaryKey({ columns: [table.reviewId, table.labelId] })
]);
